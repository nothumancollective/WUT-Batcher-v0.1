...
- Added metric-band rendering introspection coverage for component toggles (good region + warn/bad lines) and no curve-envelope path.
...
- Added KPI-context capability hints that disable warn/bad controls with Not defined for this KPI when the selected KPI spec has no matching thresholds.
...
- Final-stage smoothness and plane-consistency panels keep compact strip styling so ripple remains the visual focus.
...
- Applied the same stage-aware heatmap overlay profile to Compare heatmap rendering for visual consistency.
