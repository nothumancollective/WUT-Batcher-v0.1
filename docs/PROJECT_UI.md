...
- Builder: `ui/form_builder.py`
  - Maps field types to Qt widgets:
    ...
    - `bool -> tri-state checkbox` (`unset/true/false`)
    - `ex -> nullable line edit` (short example placeholder + tooltip semantics)
    - `list -> nullable line edit` (`e.g. 1,2,3`)
    - `object -> nested property subform` (toggle only for `Mesh.Enclosure`)
  - Uses collapsible Geometry/Mesh sections and mode subforms (`QStackedWidget`).
