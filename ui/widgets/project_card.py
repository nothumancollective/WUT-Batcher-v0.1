...
class _ProjectCardPreview(QWidget):
    ...
    def paintEvent(self, event) -> None:  # type: ignore[override]
        ...
        if not self._pixmap.isNull():
            ...
            painter.drawLine(int(rect.left()) + 30, int(rect.bottom()) - 40, int(rect.left()) + 76, int(rect.top()) + 54)
            painter.drawLine(int(rect.left()) + 76, int(rect.top()) + 54, int(rect.right()) - 42, int(rect.bottom()) - 52)
            painter.drawLine(int(rect.right()) - 42, int(rect.bottom()) - 52, int(rect.right()) - 24, int(rect.top()) + 46)
            painter.setBrush(QColor("#4A515C"))
            painter.setPen(Qt.NoPen)
            painter.drawEllipse(int(rect.left()) + 26, int(rect.top()) + 24, 10, 10)
